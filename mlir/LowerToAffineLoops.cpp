...
//===----------------------------------------------------------------------===//
// IveToAffine Conversion Patterns
//===----------------------------------------------------------------------===//
...
static Value insertAllocAndDealloc(MemRefType type, Location loc,
  ...
  // as ive functions have no control flow.
...
namespace {
//===----------------------------------------------------------------------===//
// IveToAffine Conversion Patterns: Binary operations
//===----------------------------------------------------------------------===//
...
struct ConstantOpLowering : public OpConversionPattern<ive::ConstantOp> {
  ...
  LogicalResult
  matchAndRewrite(ive::ConstantOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
//===----------------------------------------------------------------------===//
// IveToAffine Conversion Patterns: Func operations
//===----------------------------------------------------------------------===//

struct FuncOpLowering : public OpConversionPattern<ive::FuncOp> {
  using OpConversionPattern<ive::FuncOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(ive::FuncOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
//===----------------------------------------------------------------------===//
// IveToAffine Conversion Patterns: Print operations
//===----------------------------------------------------------------------===//

struct PrintOpLowering : public OpConversionPattern<ive::PrintOp> {
  using OpConversionPattern<ive::PrintOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(ive::PrintOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    // We don't lower "ive.print" in this pass, but we need to update its
    // operands.
...
//===----------------------------------------------------------------------===//
// IveToAffine Conversion Patterns: Return operations
//===----------------------------------------------------------------------===//

struct ReturnOpLowering : public OpConversionPattern<ive::ReturnOp> {
  using OpConversionPattern<ive::ReturnOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(ive::ReturnOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    ...
    // We lower "ive.return" directly to "func.return".
...
struct TransposeOpLowering : public OpConversionPattern<ive::TransposeOp> {
  using OpConversionPattern<ive::TransposeOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(ive::TransposeOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    auto loc = op->getLoc();
    lowerOpToLoops(op, rewriter, [&](OpBuilder &builder, ValueRange loopIvs) {
...
//===----------------------------------------------------------------------===//
// IveToAffineLoweringPass
//===----------------------------------------------------------------------===//

/// This is a partial lowering to affine loops of the ive operations that are
/// computationally intensive (like matmul for example...) while keeping the
/// rest of the code in the Ive dialect.
namespace {
struct IveToAffineLoweringPass
    : public PassWrapper<IveToAffineLoweringPass, OperationPass<ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(IveToAffineLoweringPass)
  StringRef getArgument() const override { return "ive-to-affine"; }
...
void IveToAffineLoweringPass::runOnOperation() {
  ...
  // a partial lowering, we explicitly mark the Ive operations that don't want
  // to lower, `ive.print`, as `legal`. `ive.print` will still need its operands
...
std::unique_ptr<Pass> mlir::ive::createLowerToAffinePass() {
  return std::make_unique<IveToAffineLoweringPass>();
}
